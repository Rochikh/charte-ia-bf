...
## 🔄 Cache : à faire à CHAQUE republication de contenu

`index.html` référence `data.js`, `styles.css`, `assets/colors_and_type.css` et les `*.compiled.js` avec un suffixe `?v=AAAAMMJJ` (ex. `data.js?v=20260708`). C'est ce qui force Teams (et les navigateurs qui gardent le cache trop longtemps) à retélécharger la nouvelle version au lieu d'afficher l'ancienne indéfiniment : sans ce paramètre, l'URL du fichier ne change jamais, donc rien n'indique au client qu'il doit revérifier.

**Dès que `data.js`, `styles.css` ou un `.compiled.js` change**, remplacer la valeur `v=...` par la date du jour (ou tout identifiant différent du précédent) à **chaque** occurrence dans `index.html`, avant de publier. Un seul fichier à modifier, une poignée de lignes.
